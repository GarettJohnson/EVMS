...
%
% [out]=woody(x,tol,max_it,est_mthd,xcorr_mthd,maxCorrLags)
%
...
% maxCorrLags   Nummber of datapoints to shift at a maximum (courtesy GDJ)
...
switch est_mthd
    ...
    case 'woody'
        out=woody_core(x,tol,max_it,xcorr_mthd,tLags);
    ...
    case 'thornton'
        ...
        for k=1:K
            ...
            if((length(ind)-2) > (M/2))
                ...
                %and exit loop
                [out,est_lags]=woody_core(x,tol,max_it,xcorr_mthd,tLags);
                break
            end
            ...
            err=1;
            while(err)
                [trash,est_lags]=woody_core(y,tol,max_it,xcorr_mthd,tLags);
...
function [out,varargout]=woody_core(x,tol,max_it,xcorr_mthd,tLags)
[N,M]=size(x);
mx=mean(x,2);
p=zeros(N,1);
conv=1;
...
while(conv*(run<max_it))
    ...
    for i=1:M,
        ...
        y=x(:,i);
        xy=xcorr(mx,y,tLags,xcorr_mthd);
        [val,ind]=max(xy);
        if(ind>ref)
            lag=ref-ind-1;
        else
